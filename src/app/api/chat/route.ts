...
async function generateHealthReport(pet: any, medicalRecords: any[], healthRecords: any[] = []): Promise<string> {
  ...
  if (bathroomRecords.length > 0) {
    ...
    if (sizeDistribution.length > 0) {
      ...
      sizeDistribution.forEach(r => {
        if (sizeCounts[r.size] !== undefined) {
          sizeCounts[r.size]++;
        }
      });
...
const sickPetKeywords = [
  '拉肚子', '拉稀', '腹泻', '呕吐', '吐了', '不吃', '不吃东西', '没精神',
  '不舒服', '生病', '病了', '感冒', '发烧', '咳嗽', '流鼻涕', '打喷嚏',
  '没力气', '没精神', '蔫了', '不对劲', '好像', '好像.*不对', '看起来.*不对劲',
  ...
  try {
    ...
    if (userId && pets.length > 0) {
      ...
      if (allergyUpdate?.allergyInfo) {
        ...
        const petIndex = pets.findIndex(p => p.id === allergyUpdate!.petId);
        if (petIndex !== -1) {
...
${allergyUpdate?.allergyInfo ? `\n已记录：${allergyUpdate.petId ? pets.find(p => p.id === allergyUpdate.petId)?.name || '宠物' : '宠物'}对${allergyUpdate.allergyInfo}过敏。` : ''}
...
• "给豆豆预约驱虫，下周"`;
    ...
    if (hasViewPetProfileIntent(lastMessage) && pets.length > 0) {
      // 提取具体宠物名称
      const mentionedPet = pets.find(p => 
...
如果需要修改信息，可以直接告诉我～`;
        ...
        const petList = pets.map(p => `• ${p.name}${p.breed ? ` (${p.breed})` : ''}`).join('\n');
...
${mentionedPet ? `${mentionedPet.name}的` : ''}日程安排已清空，如果需要安排新的日程，告诉我具体事项和时间哦～`;
    ...
    // 宠物生病检测（与户外活动互斥）
    if (hasSickPetIntent(lastMessage) && pets.length > 0 && !isOutdoorRecommendIntent && !imageData) {
      // 查找用户提到的宠物
      let sickPet = pets.find(p => 
        lastMessage.includes(p.name) || 
...
例如："给我看看球球的健康报告" 或 "分析一下豆豆最近7天的健康状况"`;
    ...
    return NextResponse.json({
      ...
      allergyUpdated: allergyUpdate?.allergyInfo ? {
        petName: allergyUpdate.petId ? pets.find(p => p.id === allergyUpdate.petId)?.name : null,
