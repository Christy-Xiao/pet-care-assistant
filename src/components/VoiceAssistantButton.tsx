...
export default function VoiceAssistantButton({ showFab = true, onActionComplete }: VoiceAssistantProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (onActionComplete) {
        onActionComplete({ result, executionResults: results });
      }
  ...
  // 取消
  const handleCancel = () => {
    if (state === 'executing') return; // 执行中不能取消
    setShowConfirmDialog(false);
    reset();
  };
  ...
  const getStateText = () => {
    switch (state) {
      ...
      case 'executing':
        const total = result?.extraction?.intents?.length || 0;
        return `执行中 ${currentExecutingIndex + 1}/${total}`;
      default: return '';
    }
  };
  ...
  return (
    <>
      {/* ====== 浮动麦克风按钮 ====== */}
      {showFab && (
        <div className="fixed bottom-6 left-[272px] z-50 flex flex-col items-center gap-2">
          {/* 状态文字 */}
      ...
      {/* ====== 结果确认/执行弹窗 ====== */}
      {showConfirmDialog && result && intents.length > 0 && (
                ...
                <div className="space-y-2">
                  ...
                  {executionResults.map((er, idx) => (
                    <div key={idx} className={`flex items-center gap-3 rounded-xl p-3 ${
                      er.success ? 'bg-green-50' : 'bg-red-50'
                    }`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-sm font-medium truncate ${er.success ? 'text-green-700' : 'text-red-600'}`}>
                          {er.message}
                        </p>
                      </div>
                      {er.success ? (
                  ...
                  {intents.map((intent, idx) => {
                    ...
                    return (
                      <div key={idx} className={`flex items-center gap-3 rounded-xl p-3 transition-all ${
                        ...
                        isDone ? (execResult?.success ? 'bg-green-50' : 'bg-red-50') :
                        'bg-gray-50'
                      }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {isDone && execResult && (
                            <p className={`text-xs truncate ${execResult.success ? 'text-green-600' : 'text-red-500'}`}>
                              {execResult.message}
                            </p>
                          )}
                        </div>
                        ...
                        <div key={idx} className={`${config.color} rounded-xl border p-3 flex items-center gap-3`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs opacity-70">
                              确信度 {Math.round((intentItem.confidence || 0) * 100)}%
                            </p>
                          </div>
            ...
            {!actionDone && state !== 'executing' && (
              <div className="px-6 py-4 border-t border-gray-100 flex gap-3 shrink-0">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl bg-primary-500 text-white font-medium hover:bg-primary-600 transition-colors flex items-center justify-center gap-2"
                >
                  <Check className="w-4 h-4" />
                  确认全部 ({intents.length}项)
                </button>
              </div>
            )}
          </div>
      ...
      {entries.map(([key, value]) => {
        ...
        return (
          <div key={key} className="flex justify-between text-xs">
            ...
            <span className="text-gray-700 font-medium max-w-[55%] truncate text-right" title={displayValue}>
              {displayValue}
            </span>
          </div>
        );
